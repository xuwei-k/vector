...
public class VectorPointer<T> {
    ...
    protected final T getElem(final int index, final int xor) {
        if (xor < (1 << 5)) { // level = 0
            ...
            return (T) (display1[(index >> 5) & 31][index & 31]);
        } else if (xor < (1 << 15)) { // level = 2
            return (T) (display2[(index >> 10) & 31][(index >> 5) & 31][index & 31]);
        } else if (xor < (1 << 20)) { // level = 3
            return (T) (display3[(index >> 15) & 31][(index >> 10) & 31][(index >> 5) & 31][index & 31]);
        } else if (xor < (1 << 25)) { // level = 4
            return (T) (display4[(index >> 20) & 31][(index >> 15) & 31][(index >> 10) & 31][(index >> 5) & 31][index & 31]);
        } else if (xor < (1 << 30)) { // level = 5
            return (T) (display5[(index >> 25) & 31][(index >> 20) & 31][(index >> 15) & 31][(index >> 10) & 31][(index >> 5) & 31][index & 31]);
        } else { // level = 6
    ...
    */
    ...
    protected final void gotoFreshPosWritable0(final int oldIndex, final int newIndex, final int xor) {
        if (xor < (1 << 5)) { // level = 0
            ...
            display4 = display5[(newIndex >> 20) & 31];
            if (display4 == null)
                display4 = (Object[][][][][]) new Object[32];
            display3 = display4[(newIndex >> 20) & 31];
            if (display3 == null)
